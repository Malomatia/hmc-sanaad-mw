...
**Sanaad** is HMC's (Hamad Medical Corporation) employee self-service mobile app. Employees use it to see their payslips, request leave, update their personal details, request HR letters, manage dependents, approve requests from their team, and more.
...
- **HMC_BackEnd** is the actual API: feature modules re-exposing legacy Sanaad operations.

**Main users:** HMC employees (mobile app).

**Key design idea to remember:** *the database is the source of truth for business logic; this codebase is a disciplined translator.* Most bugs are solved by looking at what Oracle/SQL Server actually expects.
...
```text
interface/        ← Controller + DTOs (HTTP shape, validation)
...
Cross-cutting concerns (config, database pools, JWT auth, error handling, response envelope) live once in `src/core/` and apply to every request automatically.
...
| @nestjs/axios + axios | ^4.0.1 / ^1.6.8 | Outbound HTTP (Entra Graph, SMS) & proxying | Both |
...
`npm ci` reads `package-lock.json` and installs the exact dependency tree (preferred over `npm install` for reproducibility).
...
npm run start:dev      # OR: hot-reload development server
...
The backend boots happily with every external system disabled — dependency-injection still wires up and routes register. This is how you verify your environment quickly:
...
In Postman, send `POST http://localhost:3001/api/v1/auth/login` with a JSON body containing `username`, `mpin`, `imeinumber`, `platform`, and `version`. Expect a signed JWT (dev bypass), confirming the full gateway → backend proxy path.
...
- **Why:** everything you run goes through it (`npm run build`, `npm test`).
...
### TypeScript basics
- **What:** JavaScript + types. Compiled to plain JS in `dist/` by `npm run build`.
...
All in `src/core/auth/`.

### Interceptors
Wrap around the handler (before + after). Global interceptors covered here: `TimeoutInterceptor` (408 after `REQUEST_TIMEOUT_MS`) → `ResponseInterceptor` (wraps results in the Sanaad envelope + Arabic/English localization; `@SkipEnvelope()` opts out — the auth journey uses it).

### Exception Filters
`AllExceptionsFilter` (`src/core/http/all-exceptions.filter.ts`) catches **every** error, classifies it (validation / DB / external service / auth…), and returns a safe, consistent JSON error to the client.
...
```text
development/
├── AGENTS.md                ← KEY: operational knowledge base: verified DB facts, build quirks, staging findings. READ IT.
...
```text
src/
├── main.ts                  ← bootstrap: 15mb body limit, helmet, compression, CORS, /api/v1 prefix
├── app.module.ts            ← imports CoreModule, LookupsModule + feature modules
│
...
│   ├── database/
│   │   ├── oracle.service.ts          ← THE Oracle pool: query/call/callCursor/callMultiCursor
...
│   ├── constants/oracle-objects.ts    ← KEY: allow-list of ~95 XXHMC_SND_* names — ONLY these can be queried
...
│   └── utils/   (date.util, localize.util KEY: AR/EN twin collapsing, mapper.util, blob.util, url-decode.util)
│
...
│
└── modules/                 ← feature modules, all with the same internal layout:
    ...
    └── contact/  dependents/ school-fees/ annual-ticket/ approvals/
```
...
```text
src/
├── main.ts                  ← helmet, compression, CORS, /api/v1 prefix
...
│   └── http/                ← shared axios client with backend TLS agent; minimal error filter
└── modules/
...
main.ts bootstrap()
   ...
   │        Caution: a pool that has credentials but CANNOT connect THROWS → boot fails (by design)
...
Same shape, no databases: config validation → shared axios client (with optional CA cert for an https backend) → guards/filters → listen on `GATEWAY_PORT`. Route registration order guarantees the literal `/auth/*`, `/healthcheck` routes win over the `@All('*')` wildcard.

---

# 8. Module-by-Module Documentation

> The endpoints below cover the feature modules included in this handover.
> Every route is prefixed with `/api/v1`. **Auth column:** Public = `@Public` (no token), JWT = bearer JWT required.
...
|---|---|---|---|
| POST | /healthcheck | API-1 — app-launch check (downtime + forced update) | Public |
| POST | /auth/initiate | API-2 — validate user, resolve phone, **send OTP** | Public |
| POST | /auth/otp/validate | API-3 — validate OTP | Public |
| POST | /auth/mpin/update | API-4 — set MPIN (first time) | Public |
| POST | /auth/login | API-5 — MPIN login → **JWT + refresh token + functionaccesslist** | Public |
| POST | /auth/mpin/forgot | API-6 — forgot MPIN (send OTP) | Public |
| POST | /auth/mpin/update/reset | API-7 — reset MPIN with OTP | Public |
| POST | /auth/token/refresh | exchange refresh token for a new pair (rotated, one-time use) | Public |
| POST | /auth/logout | revoke current access token (+ optional refresh token) | JWT |
| GET | /auth/me | current identity from the JWT | JWT |
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| GET | /profile?enum=&lang= | op 2 — full personal detail (5 views + dependent child rows in parallel) | JWT |
| POST | /profile/personal | op 48 — update personal details | JWT |
| GET | /profile/lov/marital-status | op 63 — marital status LOV | JWT |
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| GET | /employee/employment | op 3 — employment details + salary/assignment history | JWT |
| GET | /employee/basic | op 8 — basic info | JWT |
| GET | /employee/performance | op 7 — performance records | JWT |
| GET | /employee/supervisor/views | op 35 — supervisor candidates (returns PERSON_ID!) | JWT |
| POST | /employee/supervisor | op 36 — change supervisor (`p_new_supervisor` = PERSON_ID, not employee number) | JWT |
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| GET | /payslip/periods | op 5 — pay periods | JWT |
| GET | /payslip/count | op 6 — payslip count for a period | JWT |
| GET | /payslip | op 11 — full payslip (PAYSLIP_PR returns **7 REF CURSORs** — earnings, deductions, housing…) | JWT |
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| GET | /leave/balance?username=&lang=&effectivedate= | op 9 — leave balance (`p_user_name` bound to the request value as-is; legacy `person_id` alias accepted) | JWT |
| POST | /leave/apply | op 10 — leave submission (LeaveApplyBinds builder, attachments as base64 → BLOB) | JWT |
| POST | /leave/calculate | op 47 — duration calculation | JWT |
| POST | /leave/amend · /leave/cancel · /leave/return | ops 57/58/56 — submits | JWT |
| GET | /leaves?user_name=&leave_type= | leave history (ABSENCE_V) | JWT |
| GET | /leave/lov/types · reasons · classes · defaults · request-lov · return · return-details · return-related1 · return-related2 | ops 12/13/14/45/46/55 + RFL LOVs | JWT |
| GET | /leave/lov/cancel · /leave/lov/amend (`?person_id=` or `?username=`, optional `?leave_type=` filter on the view's NAME column) | ops 61/62 — the EXACT strings the cancel/amend submits accept | JWT |
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| GET | /letters/lov | op 16 — all letter-request LOVs (parallel fan-out) | JWT |
| POST | /letters/apply | op 17 — submit letter request | JWT |
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| GET | /identity/qid | op 18 — QID details | JWT |
| POST | /identity/qid/update | op 19 — QID update | JWT |
| POST | /identity/idcard/apply | op 54 — company ID request | JWT |
| GET | /identity/lov/work-location · delivery-location · reason | ops 53b/59/60 | JWT |
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| GET | /contact/lov/phone-type · /contact/lov/country | ops 27/30 | JWT |
| POST | /contact/phone | op 28 — upsert phones (submitted per-phone; stops at first failure) | JWT |
| POST | /contact/phone/delete | op 32 | JWT |
| POST | /contact/address · /contact/address/update | ops 29/25 — `p_country` = country NAME ("Qatar"); date-tracked (same-day repeat update fails) | JWT |
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| POST | /dependents | op 65 — add dependent (strict flexfield rules: ≥1 attachment, passport fields, unique QID, relationship from op-64 CONTACT group) | JWT |
| POST | /dependents/update · /dependents/delete | ops 24/31 | JWT |
| GET | /dependents/lov | op 64 — multi-type LOV (also serves ADDRESS_TYPE/SPONSORSHIP/VISA groups) | JWT |
| GET | /dependents/passport/types · /dependents/passport/issue-place | ops 33/49 | JWT |
| POST | /dependents/passport/apply | op 34 | JWT |
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| POST | /school-fees/apply | op 39 — school-fee request | JWT |
| GET | /school-fees/lov/schools (search/page/pageSize) · terms · edu-stage · academic-year · request-type | ops 37/38/40/50/53 | JWT |
| GET | /school-fees/children | op 52 — child details | JWT |

## 8.11 annual-ticket — ops 66, 67, 72
...
| Method | Endpoint | Description | Auth |
|---|---|---|---|
| GET | /approvals | op 20 — approvals summary | JWT |
| GET | /approvals/my-requests | op 23 | JWT |
| GET | /approvals/worklist · /approvals/worklist/summary · /approvals/worklist/{id}/history | ops 68/69/70 (keyed by USERNAME, not employee number) | JWT |
| GET | /approvals/{id}/details · /approvals/attachments/{documentId} | op 21 + attachment download | JWT |
| POST | /approvals/{id}/decision · /approvals/{id}/request-info · /approvals/{id}/reassign | ops 22 / RFMI / 71 | JWT |

## 8.13 lookups (shared kernel) — ops 15, 26 + generic reads

**Location:** `src/lookups`

| Method | Endpoint | Description | Auth |
|---|---|---|---|
| GET | /lookups/yes-no · /lookups/rfmi-user | ops 15/26 | JWT |
| GET | /lookups/lov?lovname= · /lookups/master?lookupname= | generic reads via the registry in `shared/constants/lov-names.ts` | JWT |
...
|---|---|---|---|
| POST | /healthcheck, /auth/initiate, /auth/mpin/update | forwarded verbatim | Public |
| POST | /auth/otp/validate, /auth/login, /auth/mpin/forgot, /auth/mpin/update/reset, /auth/token/refresh | forwarded verbatim + **throttled** (default 5/min per IP) | Public |
| ANY | everything else (`@All('*')`) | JWT validated locally → proxied byte-for-byte | JWT |
...
- MPIN values are stored as received (legacy decision — see §21 before changing).
...
|---|---|---|---|
| **Oracle HR** (`XXHMC_SND_*` schema) | node-oracledb pool | ~95 allow-listed views/LOVs/procedures implementing legacy business operations | `core/database/oracle.service.ts` |
...
| `readByResolvedKey(view, value, candidates)` | "filter this view by user" when the key column name varies (`USER_NAME` vs `USERNAME` vs `EMPLOYEE_NUMBER`…) | resolves the real column from ALL_TAB_COLUMNS; on schema mismatch returns `[]` instead of failing the request |
...
## 11.6 Legacy HTTP SMS gateway (only when `OTP_STORE=legacy`)
Config: `SMS_API_*`. Implemented in `sms-otp-delivery.adapter.ts` — a generic JSON POST placeholder awaiting the corporate contract. Unset base URL = hard 503 in production.

Error-handling rule for ALL integrations: adapter throws a typed exception → `AllExceptionsFilter` classifies it → client gets a safe, generic message.
...
| USERS_DB_ENCRYPT / USERS_DB_TRUST_SERVER_CERT / USERS_DB_DISABLED | no | TLS · self-signed trust · skip pool | `true`/`false`/`false` |
...
| MOTC_SMS_DB_ENCRYPT / MOTC_SMS_DB_TRUST_SERVER_CERT / MOTC_SMS_DB_DISABLED | no | TLS · trust self-signed · skip pool | `true`/`true`/`false` |
| MOTC_SMS_TABLE, MOTC_SMS_APP_ID, MOTC_SMS_FROM_ADDRESS, MOTC_SMS_SUBJECT_ID, MOTC_SMS_PRIORITY, MOTC_SMS_LANGUAGE_ID, MOTC_SMS_RECIPIENT_ADDRESS_TYPE, MOTC_SMS_PROCESSED_STATE, MOTC_SMS_MESSAGE_EXPIRE_MINUTES, MOTC_SMS_CUSTOMER_ID, MOTC_SMS_BUSINESS_PARAM1/2 | APP_ID+SUBJECT pending from client | fixed columns of the documented INSERT | see `.env.example` |
...
| REQUEST_TIMEOUT_MS / NODE_ENV | no | misc | `35000`/`development` |
...
Error path: ANY exception thrown anywhere is caught by `AllExceptionsFilter`, classified, and returned as a safe envelope: `{ success:false, message, status:'error', httpStatusCode }`.
...
2. Add the controller method with `@Get/@Post`.
...
6. `npm run build` + `npm test`, then verify by hand in Postman.
...
1. Confirm the updated columns or procedure signature with the database owners.
...
- **Backend:** 22 unit suites / ~170 tests, co-located as `src/**/*.spec.ts` (Oracle adapters with mocked pools, OTP stores, auth service, guards, utils). **No e2e specs** (`npm run test:e2e` runs nothing — the config exists, the specs were never written).
...
npx jest src/modules/leave   # one folder/file
npm run test:e2e      # gateway only (backend has no e2e specs)
```
...
1. Run `npm run start:debug` (starts ts-node-dev with `--inspect`).
...
4. Trigger the request from Postman (`postman/HMC-Sanaad-Full.postman_collection.json` has verified examples), using a bearer token from `/auth/login` for protected endpoints.

Debugging a single Jest test: `npx jest path/to.spec.ts --runInBand` with a `debugger;` statement, launched from VS Code's JavaScript Debug Terminal.

---

# 17. Important npm Commands

Same scripts in both projects (run with `npm run <name>`):
...
| `lint` | ESLint with `--fix` Caution: on the backend this crashes (known env issue) — use `npx eslint src --ext .ts` instead, and ignore the CRLF `Delete ␍` noise |
| `format` | Prettier Caution: do NOT run on the backend (CRLF working copy — it would rewrite every file). Gateway is fine. |
...
| Gateway returns `{"httpStatusCode":504}` | backend didn't answer within `BACKEND_TIMEOUT_MS` — check backend boot state and which dependency hangs |
...
| OTP endpoints return 503 | MOTC pool not created or `OTP_STORE=legacy` without SMS config |
...
| ESLint `TypeError: expand is not a function` | known env issue → `npx eslint src --ext .ts` |
...
**Day 1 — run it.** Install Node 20 + VS Code. Clone. `npm ci` in both projects. Do the §3.5 no-database smoke test, then send the §3.6 `/auth/login` request in Postman. Read `AGENTS.md` top to bottom (skim is fine).
...
**Day 3 — one full request.** With `start:debug` running and §13 next to you, follow `GET /api/v1/lookups/yes-no?lang=en` from Postman through `lookups.controller.ts → lookups.service.ts → lov.oracle.repository.ts` with breakpoints (see §16.3). Then read `core/core.module.ts` (pipeline order) and `core/http/response.interceptor.ts`.

**Day 4 — auth + data layer.** Read §9, then `modules/auth/application/auth.service.ts`, `core/auth/jwt.strategy.ts`, and the port/adapter pairs in `modules/auth`. Then §10 + `core/database/base.repository.ts` (the helpers) and one adapter (`leave.oracle.repository.ts`). Log in via Postman (`AUTH_DISABLED=true`), decode your JWT at jwt.io.

**Day 5 — make a change.** Pick something small: add a query filter to an existing LOV endpoint (mirror §15.2), write its spec, run build+tests, verify in Postman. Then debug one existing test in VS Code. Bonus: run the gateway e2e suite and read `gateway.e2e-spec.ts`.
...
Architecture, the covered endpoints and their Oracle/SQL mappings, the auth journey, configuration surface, error handling, testing, local/no-DB development, Docker build — documented above and verifiable in the repo. A large amount of *live-verified* business behavior (flexfield rules for op 65, PERSON_ID requirements for ops 36/67, date-track rules, staging DB defects) is preserved in `AGENTS.md`, DTO doc-comments, and the Postman collection's captured real responses.
...
| 4 | **Corporate HTTP SMS gateway contract** (legacy OTP path) | `SmsOtpDeliveryAdapter` is a placeholder | client |
| 5 | **Oracle/SQL schema ownership + defect process**: staging defects exist in `SCHOOL_FEE_PR` (ORA-01403), `ADD_OR_UPDATE_PHONE` (rejects all phone types), `UPDATE_DEPENDENT_PR` (intermittent ORA-00027) — correct requests fail inside the DB | you'll be blamed for DB bugs otherwise | Oracle apps team |
| 6 | **Business rules not encoded anywhere**: op-51 (skipped — why?), exact role assignment rules (who gets APPROVER), MPIN client-side hashing algorithm | product correctness | client / mobile team |
| 7 | **Mobile app team contact + release coordination**: response-shape changes must be coordinated (e.g. HTTP-200-on-business-failure convention, addressType twin exception) | breaking the app silently | mobile team |
| 8 | **Security review decisions**: MPIN stored as received (legacy equality), in-memory token revocation, `trustServerCertificate` usage — accepted trade-offs that may need revisiting | compliance | previous developer / security |
| 9 | **docker-compose env gap**: newer vars (MOTC_SMS_*, OTP_STORE, AUTH_STATIC_LOGIN) are not in the compose `environment:` pass-through — decide whether to add them or manage via `.env` on servers | deployment surprises | DevOps |
| 10 | **Git remote / branching / code review process** — the repos have no CI config; the workflow is undocumented | collaboration | previous developer |
