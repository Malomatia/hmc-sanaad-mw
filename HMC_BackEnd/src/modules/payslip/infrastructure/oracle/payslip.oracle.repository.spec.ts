  ...
  it.each(['en', 'ar'] as const)('localizes FLEX errors for %s', async (lang) => {
    const ora = {
      callMultiCursor: jest.fn().mockResolvedValue({
        cursors: {},
        scalars: { p_success_flag: 'N', p_error_msg: 'ORA-20001: flex validation failed' },
      }),
    } as unknown as OracleService;
    const repository = new PayslipOracleRepository(ora, {} as OracleSchemaService);
    const result = await repository.generate({
      personId: '123', lang, payPeriod: 'September 2026', assignmentId: '456',
    });
